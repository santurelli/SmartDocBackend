...
import org.apache.commons.text.StringSubstitutor; // Updated to commons-text
...
public class ClientiDao extends BaseDao {
    ...
    public List<ClienteDto> getList(String strToSearch,
        ...
        if (orderColumn == 0) {
            ...
            valuesMap.put("ORDER_BY",
                    new StringBuilder("COALESCE(d_e_indirizzi_clienti.citta, '') ").append(orderDir).toString());
        } else if (orderColumn == 3) {
            valuesMap.put("ORDER_BY",
                    new StringBuilder("COALESCE(d_e_clienti.referente, '') ").append(orderDir).toString());
        } else if (orderColumn == 4) {
            valuesMap.put("ORDER_BY",
                    new StringBuilder(
                            "COALESCE(partita_iva || '/' || codice_fiscale, partita_iva, codice_fiscale, '') ")
                            .append(orderDir).toString());
        } else if (orderColumn == 5) {
            valuesMap.put("ORDER_BY",
                    new StringBuilder(" COALESCE(d_e_agenti.denominazione, '') ").append(orderDir).toString());
        } else {
            valuesMap.put("ORDER_BY", new StringBuilder("denominazione ").append(orderDir).toString());
        }
        ...
        query = StringSubstitutor.replace(query, valuesMap); // Updated to StringSubstitutor
        try {
            BeanPropertyRowMapper<ClienteDto> rowMapper = new BeanPropertyRowMapper<>();
            rowMapper.setMappedClass(ClienteDto.class);
    ...
    public List<ClienteDto> getSuggestion(String query) throws SQLException {
        try {
            BeanPropertyRowMapper<ClienteDto> rowMapper = new BeanPropertyRowMapper<>();
            rowMapper.setMappedClass(ClienteDto.class);
            return jdbcTemplate.query(FileQueryReader.getQuery("CLIENTI_S06"), rowMapper,
                    StringUtility.formatForLike(query.toLowerCase()), StringUtility.formatForLike(query.toLowerCase()),
    ...
    public Integer insert(ClienteDto dto) throws SQLException {
        try {
            return jdbcTemplate.queryForObject(FileQueryReader.getQuery("CLIENTI_I01"), Integer.class,
                    StringUtils.defaultIfEmpty(dto.getCodice(), null), dto.getTipologia().name(),
                    StringUtils.defaultIfEmpty(dto.getDenominazione(), null),
                    StringUtils.defaultIfEmpty(dto.getReferente(), null),
                    StringUtils.defaultIfEmpty(dto.getCodiceFiscale(), null),
                    StringUtils.defaultIfEmpty(dto.getPartitaIva(), null),
                    StringUtils.defaultIfEmpty(dto.getNote(), null), dto.getIdTipoPagamento(),
                    StringUtils.defaultIfEmpty(dto.getCodSia(), null),
                    ...
                    dto.getIdRisorsa(), dto.getIdAliquotaIva(), dto.getIdVettore(), dto.getIdTipoPorto(),
                    dto.getDocumentiMail(), dto.getIdAvviso(), dto.getIdNota(), dto.getIdAgente(), dto.getIdListino(),
                    dto.getSconto(), dto.getIdZonaCompetenza(), dto.getIdSottoconto(), dto.getIdLingua(),
                    dto.getUserCreated());
        } catch (DataAccessException e) {
    ...
    public boolean isExistentCodice(String codice,
        ...
        try {
            ...
            _log.error("Errore nella determinazione dell'esistenza del cliente con codice {} e id {}", codice,
                    id == null ? "<nullo>" : id, e);
            throw new SQLException(e);
        }
    }
    ...
    public boolean isExistentDenominazione(String denominazione,
            Integer id) throws SQLException {
        try {
            long l = jdbcTemplate.queryForObject(FileQueryReader.getQuery("CLIENTI_S05"), Long.class, denominazione,
                    id);
            return l > 0;
        } catch (DataAccessException e) {
            _log.error("Errore nella determinazione dell'esistenza del cliente con denominazione {} e id {}",
                    denominazione, id == null ? "<nullo>" : id, e);
            throw new SQLException(e);
        }
    }
    ...
    public void update(ClienteDto dto) throws SQLException {
        try {
            jdbcTemplate.update(FileQueryReader.getQuery("CLIENTI_U01"),
                    StringUtils.defaultIfEmpty(dto.getCodice(), null), dto.getTipologia().name(),
                    StringUtils.defaultIfEmpty(dto.getDenominazione(), null),
                    StringUtils.defaultIfEmpty(dto.getReferente(), null),
                    StringUtils.defaultIfEmpty(dto.getCodiceFiscale(), null),
