...
public class FornitoriDao extends BaseDao {
    ...
    private static final Log _log = LogFactory.getLog(FornitoriDao.class);
    ...
    public List<FornitoreDto> getList(String strToSearch, Integer length, Integer start, Integer orderColumn,
        ...
        if (orderColumn == 0) {
            ...
            valuesMap.put("ORDER_BY",
                    new StringBuilder("COALESCE(partita_iva || '/' || codice_fiscale, partita_iva, codice_fiscale, '') ")
    ...
    public long insert(FornitoreDto dto) throws SQLException {
        try {
            return jdbcTemplate.queryForObject(FileQueryReader.getQuery("FORNITORI_I01"), Long.class,
                    StringUtils.defaultIfEmpty(dto.getCodice(), null),
                    ...
                    dto.getIdRisorsa(), dto.getIdAliquotaIva(), dto.getIdCategoriaSpesa(), dto.getIdVettore(),
                    dto.getIdTipoPorto(), dto.getDocumentiMail(), dto.getIdAvviso(), dto.getIdNota(),
                    dto.getUserCreated());
        } catch (DataAccessException e) {
    ...
    public boolean isExistentCodice(String codice, Integer id) throws SQLException {
        try {
            ...
            _log.error("Errore nella verifica dell'esistenza del codice fornitore con codice " + codice + " e id "
                    + (id == null ? "<nullo>" : id), e);
            throw new SQLException(e);
        }
    }
    ...
    public boolean isExistentDenominazione(String denominazione, Integer id) throws SQLException {
        try {
            long l = jdbcTemplate.queryForObject(FileQueryReader.getQuery("FORNITORI_S05"), Long.class, denominazione,
                    id);
            return l > 0;
        } catch (DataAccessException e) {
            _log.error("Errore nella verifica dell'esistenza del codice fornitore con denominazione " + denominazione
                    + " e id " + (id == null ? "<nullo>" : id), e);
            throw new SQLException(e);
        }
    }
    ...
    public void update(FornitoreDto dto) throws SQLException {
        try {
            jdbcTemplate.update(FileQueryReader.getQuery("FORNITORI_U01"),
                    StringUtils.defaultIfEmpty(dto.getCodice(), null),
                    StringUtils.defaultIfEmpty(dto.getDenominazione(), null),
                    StringUtils.defaultIfEmpty(dto.getReferente(), null),
                    StringUtils.defaultIfEmpty(dto.getCodiceFiscale(), null),
