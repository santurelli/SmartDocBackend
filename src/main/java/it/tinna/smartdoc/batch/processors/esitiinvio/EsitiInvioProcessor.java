package it.tinna.smartdoc.batch.processors.esitiinvio;
...
public class EsitiInvioProcessor implements ItemProcessor<File, File>
{
    ...
    {
        return new File(stepExecution.getJobExecution().getExecutionContext().getString(BatchConstants.EXECUTIONCONTEXT_JOBDIR), FilenameUtils.getName(arg0.getAbsolutePath()));
...
//                logger.info("Comando di decifratura {} terminato con codice di uscita {}", out.toString(), exitValue);
...
//                    template1.process(model, out);
//                    logger.info("Comando di verifica firma generato: {}", out.toString());
//                    cmdLine = CommandLine.parse(out.toString());
//                    try
//                    {
//                        exitValue = executor.execute(cmdLine);
//                        logger.debug("Comando di verifica firma {} terminato con codice di uscita {}", out.toString(), exitValue);
//                        return fileFirmaVerificata;
//                    }
//                    catch ( IOException e )
//                    {
//                        logger.error("Errore nell'esecuzione del comando di verifica firma {}", out.toString(), e);
//                        throw e;
//                    }
//                }
//                catch ( IOException e )
//                {
//                    logger.error("Errore nella generazione del comando di verifica firma", e);
//                    throw e;
//                }
//            }
//            catch ( IOException e )
//            {
//                logger.error("Errore nell'esecuzione del comando di decifratura {}", out.toString(), e);
//                throw e;
//            }
//        }
//        catch ( IOException e )
...
//        }
        ...
        // {
        // int exitValue = executor.execute(cmdLine);
        // logger.debug("Comando di decrifatura {} del file esito invio terminato con codice di uscita {}", strBCommand.toString(), exitValue);
        // // verifico firma
        // File fileFirmaVerificata = new File(workDir, FilenameUtils.getBaseName(fileDecifrato.getAbsolutePath()));
